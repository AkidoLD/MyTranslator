...
class StackFrame(Frame):
    ...
    def add_stack_item(self, tag: str, **kwargs) -> Frame:
        ...
        #Store the frame to keep above if not set
        if not self.raised_stack :
            self.raised_stack = frame
        frame.place(relx=0, rely=0, relwidth=1, relheight=1)
        self.raised_stack.tkraise()
        return frame
